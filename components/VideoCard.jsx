...
<div className="p-4 mx-2 my-2 md:w-[31%] rounded-lg bg-black/10 ">
    ...
    <Link key={video.snippet.publishedAt} href={`${process.env.PRODUCTION_URL}/${video.id.videoId  ? 'watch' : 'channel' }/${video.id.videoId ? video.id.videoId:video.id.channelId }`}><Image   width={500} height={500} className={`lg:h-48 md:h-36 w-full object-cover object-center ${video.id.channelId && "md:h-1/2 m-auto h-1/2  w-1/2 rounded-full"} `}  src={video?.snippet?.thumbnails?.high?.url} alt="Loading..." /></Link>
      <div className="p-2">
