...
const SearchVideoCard = ({video}) => {
  ...
  return (<>
    ...
    {video.map((item)=>{
      ...
      <Link href={`${process.env.PRODUCTION_URL}/${item.id.videoId  ? 'watch' : 'channel' }/${item.id.videoId ? item.id.videoId:item.id.channelId }`}><Image src={item?.snippet?.thumbnails?.high?.url} width={500} height={500}  alt="loading" className={`object-c  w-full h-full  ${item.id.channelId && "w-1/5 m-auto rounded-full"}`} /></Link>
