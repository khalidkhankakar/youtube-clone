...
const mainVid = await Cvideos.items;
  ...
  return (
    <>
      <section class="channel-banner flex-shrink-0 w-full h-[14rem] justify-end items-end bg-cover bg-center p-4" style={{ backgroundImage: `url(${mainData.brandingSettings.image.bannerExternalUrl})` }} />
      ...
      <section className="bg-black/10 mt-1 flex flex-col py-2 justify-center sm:items-center md:flex-row md:justify-between">
        <div className="logo flex gap-4 flex-col md:flex-row justify-center items-center">
          ...
          <div className="deatail flex flex-col gap-2">
            <h1 className="text-2xl font-medium md:text-start sm:text-center">{mainData.snippet.title}</h1>
            <div><span className="font-light sm:text-center text-blue-600 cursor-pointer hover:underline">{mainData.snippet.customUrl}</span>  <span className="font-medium text-red-600 sm:text-center">{formatViews(mainData.statistics.subscriberCount)} subscribers</span> <span className="font-medium sm:text-center">{formatViews(mainData.statistics.videoCount)} videos</span></div>
            <p>{mainData.snippet.description.substr(0, 100)}</p>
          </div>
        </div>
        <div className="subscribe">
          <h1 className="text-lg cursor-pointer text-white hover:font-semibold rounded-full mx-7 mt-3 py-1 px-3 bg-black/40">Subscribe</h1>
        </div>
      </section>
      <div class="featured-video  flex cursor-pointer my-2">
        ...
        <div class="video-info flex-1">
          ...
          <ul class="video-meta-data">
            <li class="video-upload-date">{getTimeAgo(mainVid[0].snippet.publishedAt)}</li>
          </ul>
        </div>
      </div>
      ...
      <div key={item.snippet.publishedAt} class="p-2 md:w-1/4">
        <div class="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
          ...
          <div class="p-2">
            ...
            <span class="inline-block py-1 px-2 text-gray-700 text-xs font-medium tracking-widest">{getTimeAgo(item.snippet.publishedAt)}</span>
            </div>
          </div>
        </div>
      </div>    ))}
